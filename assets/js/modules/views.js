...
function openCompetencyDetails(competencyId) {
  ...
  dialog.innerHTML = `<article class="competency-dialog-card"><header class="competency-dialog-header"><div><p class="eyebrow">${escapeHtml(category?.nome || "")} · ${escapeHtml(domain?.nome || "")}</p><h2>${escapeHtml(competency.nome)}</h2><p class="muted">${escapeHtml(competency.descricao || "")}</p></div><button class="icon-button" data-close-dialog>×</button></header>
    ...
    <section class="levels-readonly"><h3>Níveis da competência</h3>${levels.map(level => `<article class="level-readonly-card"><div><strong>${escapeHtml(level.nome)}</strong><p class="muted">${escapeHtml(level.descricao || "")}</p></div><div class="resource-list resource-list-clean">${dataService.getResourcesForLevel(level.nivel_id).map(resource => renderResourceCard(resource, resourceTypes, competencyId)).join("") || '<div class="empty-state compact">Nenhum recurso publicado.</div>'}</div></article>`).join("")}</section></article>`;
  document.body.appendChild(dialog);
  ...
  dialog.querySelectorAll(".resource-status-select").forEach(select => select.addEventListener("change", event => { userDataService.setResourceStatus(event.currentTarget.dataset.resourceId, event.currentTarget.value, competencyId); const suggestion=userDataService.getCycleSuggestion(competencyId); if(suggestion==="CICLO_ENCERRADO" && window.confirm("Todos os recursos planejados foram concluídos. Deseja encerrar este ciclo de estudo?")) userDataService.setCompetencyStatus(competencyId,"CICLO_ENCERRADO"); else if(suggestion==="PAUSADA" && window.confirm("Todos os recursos planejados foram cancelados. Deseja pausar este ciclo?")) userDataService.setCompetencyStatus(competencyId,"PAUSADA"); }));
...
function renderResourceCard(resource, resourceTypes, competencyId) {
  const progress = userDataService.getResourceProgress(resource.recurso_id, competencyId);
  return `<article class="resource-card resource-card-clean"><div class="resource-main-copy"><span class="resource-type">${escapeHtml(resourceTypes.get(resource.tipo_recurso_id) || resource.tipo_recurso || "Recurso")}</span><h4>${escapeHtml(resource.nome)}</h4><p class="muted">${escapeHtml(resource.descricao || "")}</p>${progress?.targetDate ? `<small>Meta: ${formatDate(progress.targetDate)}</small>` : ""}</div><div class="resource-actions"><label class="filter-field"><span>Status</span><select class="resource-status-select" data-resource-id="${escapeAttribute(resource.recurso_id)}">${resourceStatusOptions(progress?.status || "")}</select></label><button class="button button-secondary resource-details-button" data-resource-id="${escapeAttribute(resource.recurso_id)}">Datas</button>${(resource.url_principal || resource.url) ? `<a class="button button-secondary" href="${escapeAttribute(resource.url_principal || resource.url)}" target="_blank" rel="noopener noreferrer">Abrir</a>` : ""}</div></article>`;
}
...
function openTrailDetails(trailId) {
    ...
    <article class="competency-dialog-card stack">
      ...
      <section class="stack">
        ${(analysis.items || []).map(item => {
          ...
          const isPlanned = progress?.status && !["CANCELADA","CICLO_ENCERRADO"].includes(progress.status);
  ...
  dialog.querySelectorAll(".trail-plan-competency:not([disabled])").forEach(button => {
    button.addEventListener("click", () => {
      userDataService.setCompetencyStatus(button.dataset.competencyId, "A_INICIAR");
...
function renderPlanning() {
  ...
  const categories = dataService.getAll("categorias", { activeOnly: true });
  const columns = [["BACKLOG","Backlog"],["A_INICIAR","A iniciar"],["EM_ANDAMENTO","Em andamento"],["PAUSADA","Pausada"]];
...
function planningCompetencyCard(item) {
  const visibleResources = item.status === "CICLO_ENCERRADO" ? item.resources.filter(r=>r.status) : item.resources.filter(r=>["EM_ABERTO","EM_ANDAMENTO"].includes(r.status));
...
function competencyStatusLabel(status){return ({BACKLOG:"Backlog",A_INICIAR:"A iniciar",EM_ANDAMENTO:"Em andamento",PAUSADA:"Pausada",CICLO_ENCERRADO:"Ciclo encerrado",CANCELADA:"Cancelada"})[status]||"Fora do planejamento";}
...
function competencyPlanningOptions(selected=""){return [["","Fora do planejamento"],["BACKLOG","Backlog"],["A_INICIAR","A iniciar"],["EM_ANDAMENTO","Em andamento"],["PAUSADA","Pausada"],["CICLO_ENCERRADO","Ciclo encerrado"],["CANCELADA","Cancelada"]].map(([v,l])=>`<option value="${v}" ${v===selected?"selected":""}>${l}</option>`).join("");}
function resourceStatusOptions(selected=""){return [["","Sem status"],["EM_ABERTO","Em aberto"],["EM_ANDAMENTO","Em andamento"],["CONCLUIDO","Concluído"],["CANCELADO","Cancelado"]].map(([v,l])=>`<option value="${v}" ${v===selected?"selected":""}>${l}</option>`).join("");}" ${v===selected?"selected":""}>${l}</option>`).join("");}
